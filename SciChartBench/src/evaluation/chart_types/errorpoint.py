"""
Errorpoint / errorbar parser — ErrorRow format.

y_value must be a dict with keys: min, median, max (any subset accepted).
"""
from __future__ import annotations
from pathlib import Path
from ._helpers import to_float, warn, ranges_auto, _is_horizontal, _normalize_label
from ..row_types import ChartRow, ErrorRow, MetaRow
...
def parse(data: dict, filepath: str | Path | None = None) -> list[ChartRow]:
    ...
    for dp in data.get("data_points", []):
        ...
        rows.append(ErrorRow(
            ...
            min=_f("min"), median=_f("median"), max=_f("max"),
        ))
...
def show_table(data: dict) -> str:
    rows = [r for r in parse(data) if isinstance(r, ErrorRow)]
    if not rows:
        return "(no data)"
    w_s = max((len(r.series) for r in rows), default=6)
    w_l = max((len(r.label) for r in rows), default=5)
    w_s, w_l = max(w_s, 6), max(w_l, 5)
    header = f"{'Series':<{w_s}}  {'Label':<{w_l}}  {'min':>8}  {'median':>8}  {'max':>8}"
    sep = "-" * len(header)
    lines = [header, sep]
    for r in rows:
        def _fmt(v): return f"{v:>8.4f}" if v is not None else "    None"
        lines.append(f"{r.series:<{w_s}}  {r.label:<{w_l}}  {_fmt(r.min)}  {_fmt(r.median)}  {_fmt(r.max)}")
    return "\n".join(lines)
